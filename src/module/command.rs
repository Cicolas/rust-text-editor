...
impl CommandModule {
    ...
    fn trigger_actions(&mut self, actions: &Vec<Action>) -> Option<Vec<OutcomingConsoleEvent>> {
        ...
        for action in actions.into_iter() {
            match action {
                Action::Move(movement) => match movement {
                    Movement::Left => {
                        self.render_col = cmp::max(0, (self.render_col as i32) - 1) as u32;
                    }
                    Movement::Right => {
                        self.render_col =
                            cmp::min(self.command_str.len() as u32, self.render_col + 1) as u32;
                    }
                ...
                Action::Delete => {
                    if self.command_str.len() > 0 {
                        ...
                        self.render_col =
                            cmp::min(self.command_str.len() as u32, self.render_col) as u32;
                    }
                }
                Action::Quit => {
                    return_vec.push(OutcomingConsoleEvent::UnfocusMe);
                }
...
impl ModuleEvent for CommandModule {
    fn proxy_trigger_events(
        &mut self,
        event: &IncomingConsoleEvent,
        idx: u32
    ) -> Option<Vec<OutcomingConsoleEvent>> {
        match event {
            IncomingConsoleEvent::Key(KeyEvent { code, .. }) if *code == KeyCode::Char(':') => {
                Some(vec![
    ...
    fn on_draw(&self) -> Option<Vec<crate::client::DrawAction>> {
        ...
        Some(vec![
            DrawAction::AskRedraw(Redraw::Line(0, str)),
